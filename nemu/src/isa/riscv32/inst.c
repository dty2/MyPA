...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //assembler pseudoinstruction! addi INSTPAT("??????? ????? ????? 000 ????? 00100 11", li     , I, R(dest) = imm);
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = imm + s->pc);
  //assembler pseudoinstruction! addi INSTPAT("??????? ????? ????? 000 ????? 00100 11", mv     , I, R(dest) = src1);
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = (unsigned)src1 < 0 ? 1 : 0);//attention ! unsigned!
  //assembler pseudoinstruction! sltiu INSTPAT("??????? ????? ????? 010 ????? 00100 11", seqz   , I, R(dest) = (unsigned)src1 < imm ? 1 : 0);//attention ! unsigned!
																				  
  //add code for other.c for second time
  /*
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(dest) = src1 < imm ? 1 : 0);
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", seqz   , I, R(dest) = (unsigned)src1 < imm ? 1 : 0);//attention ! unsigned!
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = (unsigned)src1 < 0 ? 1 : 0);//attention ! unsigned!
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = imm + s->pc; R(dest) = s->snpc;);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , R, R(dest) = src1 + src2);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , R, R(dest) = src1 + src2);
