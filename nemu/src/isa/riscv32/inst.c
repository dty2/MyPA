...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = (int32_t)imm + s->pc; R(dest) = s->snpc);//assembler pseudoinstruction: j
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr    , R, R(dest) = s->dnpc = imm + src1; R(dest) = s->snpc);//assembler pseudoinstruction: ret

  //add code for add
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(dest) = (uint32_t)(src1 + src2));
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = (uint32_t)(src1 - src2));//assembler pseudoinstruction:neg(negitive)
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = (uint32_t)src1 < (uint32_t)imm);//attention ! unsigned! assembler pseudoinstruction:seqz
  ...
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(dest) = (uint32_t)src1 < (uint32_t)src2);//attention ! unsigned!
  ...
  //add code for swith
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu    , B, R(dest) = (uint32_t)src1 < (uint32_t)src2 ? imm + s->pc : s->dnpc);
