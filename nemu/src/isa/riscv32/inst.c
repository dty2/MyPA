...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = (int)src1 == (int)src2 ? imm + s->pc : s->dnpc); //assembler pseudoinstruction:beqz
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = (int)src1 != (int)src2 ? imm + s->pc : s->dnpc);
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = (int)src1 >= (int)src2 ? imm + s->pc : s->dnpc);
