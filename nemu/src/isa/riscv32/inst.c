...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //add code for bit
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 1, src2 << 16 >> 16));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = ((int)src1) >> imm);
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + imm, 4) << 24 >> 24);
  ...
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2 << 24 >> 24));
