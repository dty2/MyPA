...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //add code for dummy
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src1 + (int)imm);//assembler pseudoinstruction: li, mv
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = (int)imm + s->pc);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = (int)imm + s->pc; R(dest) = s->snpc);//assembler pseudoinstruction: j
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", ret    , I, s->dnpc = R(1));

  //add code for add
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(dest) = (int)src1 + (int)src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = (int)src1 - (int)src2);//assembler pseudoinstruction:neg(negitive)
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = (unsigned int)src1 < (unsigned int)imm);//attention ! unsigned! assembler pseudoinstruction:seqz
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = (int)src1 == (int)src2 ? (int)imm + s->pc : s->dnpc); //assembler pseudoinstruction:beqz
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = (int)src1 != (int)src2 ? (int)imm + s->pc : s->dnpc);

  //add code for add-longlong
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(dest) = (unsigned int)src1 < (unsigned int)src2);//attention ! unsigned!
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(dest) = (int)src1 ^ (int)src2);
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(dest) = (int)src1 | (int)src2);

  //add code for bit
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + imm, 2, src2));
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (int)src1 >> (int)imm);
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = (int)src1 & (int)imm);
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(dest) = (int)src1 << (int)src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = (int)src1 & (int)src2);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = (int)src1 ^ (int)imm);
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + imm, 1, src2));

  //add code for bubble-sort
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = (int)src1 >= (int)src2 ? (int)imm + s->pc : s->dnpc);
  
  //add code for div
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = (int)src1 * (int)src2);
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = (int)src1 / (int)src2);

  //add code for doldbach
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(dest) = src1 % src2);

  //add code for if-else
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = (int)src1 < (int)src2 ? (int)imm + s->pc : s->dnpc);
