...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = (int32_t)imm + s->pc; R(dest) = s->snpc; ftrace_get_jump(s->pc, s->dnpc, 0));//assembler pseudoinstruction: j
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = imm + src1; R(dest) = s->snpc; ftrace_get_jump(s->pc, s->dnpc, 1));//assembler pseudoinstruction: ret
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = src1 == src2 ? imm + s->pc : s->dnpc; ftrace_get_jump(s->pc, s->dnpc, 0)); //assembler pseudoinstruction:beqz
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = src1 != src2 ? imm + s->pc : s->dnpc; ftrace_get_jump(s->pc, s->dnpc, 0));
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = (int32_t)src1 >= (int32_t)src2 ? imm + s->pc : s->dnpc; ftrace_get_jump(s->pc, s->dnpc, 0));
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = (int32_t)src1 < (int32_t)src2 ? imm + s->pc : s->dnpc; ftrace_get_jump(s->pc, s->dnpc, 0));
  ...
  //add code for switch
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, s->dnpc = src1 < src2 ? imm + s->pc : s->dnpc; ftrace_get_jump(s->pc, s->dnpc, 0));
  
  //add code for string
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s->dnpc = src1 >= src2 ? imm + s->pc : s->dnpc; ftrace_get_jump(s->pc, s->dnpc, 0));
