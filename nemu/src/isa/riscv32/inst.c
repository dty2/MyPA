...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = (int32_t)imm + s->pc; R(dest) = s->snpc; /*ftrace_get_jump(s->pc, s->dnpc)*/);//assembler pseudoinstruction: j
  ...
  //add code for div
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = (int32_t)src1 * (int32_t)src2);
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, if(!src2) R(dest) = 0; else R(dest) = (int32_t)src1 % (int32_t)src2);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(dest) = src1 % src2);

  //add code for dhrystone
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(dest) = src1 < (int32_t)imm ? 1 : 0);
  /*
