...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //add code for dummy
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = (uint32_t)(src1 + (int32_t)imm));//assembler pseudoinstruction: li, mv
  ...
  //add code for div
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = (uint32_t)(src1 * src2));
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = (uint32_t)(src1 / src2));
  ...
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = Mr(src1 + imm, 1));
