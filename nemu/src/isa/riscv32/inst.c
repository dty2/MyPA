...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = imm + s->pc; R(dest) = s->pc + 4; /*ftrace_get_jump(s->pc, s->dnpc)*/);//assembler pseudoinstruction: j
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = imm + src1; R(dest) = s->pc + 4; /*ftrace_get_jump(s->pc, s->dnpc)*/);//assembler pseudoinstruction: ret

  //add code for add
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(dest) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = src1 - src2);//assembler pseudoinstruction:neg(negitive)
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = src1 < (word_t)imm);//attention ! unsigned! assembler pseudoinstruction:seqz change!!!
  ...
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (int)src1 >> (int)imm);//change!!!
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = (int)src1 / (int)src2);//change!!!

  //add code for doldbach
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(dest) = (int)src1 % (int)src2);//change!!!

  //add code for if-else
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = (int)src1 < (int)src2 ? imm + s->pc : s->dnpc;);//change!!!
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(dest) = (int)src1 < (int)src2);//change!!!
  ...
  //I solve this problem! emmm, maybe.
