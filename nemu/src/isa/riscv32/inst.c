...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = src1 >= src2 ? imm + s->pc : s->dnpc;);
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = src1 < src2 ? imm + s->pc : s->dnpc;);
  ...
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, if(!src2) R(dest) = 0; else R(dest) = src1 % src2);
