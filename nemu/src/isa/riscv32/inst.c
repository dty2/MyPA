...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //add code for dummy
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src1 + (signed)imm);//assembler pseudoinstruction: li, mv
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = imm + s->pc);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = (signed)imm + s->pc; R(dest) = s->snpc);//assembler pseudoinstruction: j
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = (unsigned)src1 < (unsigned)imm);//attention ! unsigned! assembler pseudoinstruction:seqz
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = src1 == src2 ? imm + s->pc : s->dnpc); //assembler pseudoinstruction:beqz
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = src2 != src2 ? imm + s->pc : s->dnpc);

  //add code for add-longlong
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(dest) = (unsigned)src1 < (unsigned)src2);//attention ! unsigned!
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = (signed)src1 >= (signed)src2 ? imm + s->pc : s->dnpc);
  ...
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = (signed)src1 < (signed)src2 ? imm + s->pc : s->dnpc);
  ...
  INSTPAT("0000000 ????? ????? 101 ????? 00100 11", srli    , R, R(dest) = src1 >> imm);
