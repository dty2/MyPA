...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = src1 < imm ? 1 : 0);//attention ! unsigned! assembler pseudoinstruction:seqz
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = src1 == src2 ? imm + s->pc : s->snpc); //assembler pseudoinstruction:beqz
