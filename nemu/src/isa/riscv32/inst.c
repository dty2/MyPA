...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //add code for dummy
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = (uint32_t)((int32_t)src1 + (int32_t)imm));//assembler pseudoinstruction: li, mv
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = imm + s->pc);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = (uint32_t)((int32_t)imm + (int32_t)s->pc); R(dest) = s->snpc; /*ftrace_get_jump(s->pc, s->dnpc)*/);//assembler pseudoinstruction: j
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (uint32_t)((int32_t)imm + (int32_t)src1); R(dest) = s->snpc; /*ftrace_get_jump(s->pc, s->dnpc)*/);//assembler pseudoinstruction: ret
  ...
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = (uint32_t)((int32_t)src1 & (int32_t)imm));
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(dest) = src1 << src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = (uint32_t)((int32_t)src1 ^ (int32_t)imm));
  ...
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = SEXT_MY((int16_t)Mr(src1 + imm, 2), 16));
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(dest) = Mr(src1 + imm, 2));
