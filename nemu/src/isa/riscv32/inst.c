...
//add code for elf
extern void ftrace_get_jump(int, int);
//add end
...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src1 + imm);//assembler pseudoinstruction: li, mv change!!!
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = Mr(src1 + imm, 4));
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = imm + src1; R(dest) = s->snpc;);//assembler pseudoinstruction: ret
  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = src1 < imm);//attention ! unsigned! assembler pseudoinstruction:seqz
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = (int32_t)src1 >> (int32_t)imm);//change!!!
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = Mr(src1 + imm, 1));
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = src1 & imm);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xori   , I, R(dest) = src1 ^ imm);//change!!!
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = SEXT(Mr(src1 + imm, 2), 16));//change!!!
  ...
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(dest) = SEXT(Mr(src1 + imm, 1), 8));// change!!!
  INSTPAT("??????? ????? ????? 010 ????? 00100 11", slti   , I, R(dest) = (int32_t)src1 < (int32_t)imm);
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(dest) = src1 | imm);//change!!!
  ...
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = (int32_t)src1 >= (int32_t)src2 ? imm + s->pc : s->dnpc;);//change!!!
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = (int32_t)src1 < (int32_t)src2 ? imm + s->pc : s->dnpc;);//change!!!
  ...
  INSTPAT("0000001 ????? ????? 100 ????? 01100 11", div    , R, R(dest) = (int32_t)src1 / (int32_t)src2);//change!!!
  INSTPAT("0000001 ????? ????? 110 ????? 01100 11", rem    , R, R(dest) = (int32_t)src1 % (int32_t)src2);//change!!!
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(dest) = (int32_t)src1 < (int32_t)src2);//change!!!
  INSTPAT("0000001 ????? ????? 001 ????? 01100 11", mulh   , R, R(dest) = (int32_t)SEXT((int64_t)src1, 32) * SEXT((int64_t)src2, 32) >> 32);
  INSTPAT("0100000 ????? ????? 101 ????? 01100 11", sra    , R, R(dest) = (int32_t)src1 >> (int32_t)src2);//change!!!
  INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srl    , R, R(dest) = src1 >> src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(dest) = src1 / src2);
  INSTPAT("0000001 ????? ????? 011 ????? 01100 11", mulhu  , R, R(dest) = ((int64_t)src1 * (int64_t)src2) >> 32);
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(dest) = src1 % src2);
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(dest) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = src1 - src2);//assembler pseudoinstruction:neg(nagitive)
