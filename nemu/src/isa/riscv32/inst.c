...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , R, s->dnpc = imm + src1; R(dest) = s->snpc);//assembler pseudoinstruction: ret
  ...
  //add code for switch
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, R(dest) = (unsigned)src1 < (unsigned)src2 ? imm + s->pc : s->dnpc);
